...
def flush(progress_bar):
    ...
    with ThreadPoolExecutor(max_workers=8) as ThreadPool:
        ...
        max_page = 2
        online_urls = [host + f"/action/doSearch?SeriesKey=14754762&sortBy=Earliest&pageSize=50&startPage={page}" for page in range(max_page)]
        ...
        try:
            database_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data\\Area.db')
            ...
            for id, article in issue_newest_article_lst:
                if not my_data_manager.search_data(newest_issue, article.title):
                    ...
                    print(f'{article.title:<200} |已存在于 Area_issue')
            print('Area数据存储成功！')
            ...
            for id, article in online_newest_article_lst:
                if not my_data_manager.search_data('online', article.title):
                    ...
                    print(f'{article.title:<200} |已存在于 Area_online')
            print('Area_online数据存储成功！')
        except Exception as e:
            print(e)
