...
class SubCrawl():
    ...
    def parse_issue(self, text):
        '''The_Geographical_Journal `issue`'''
        # try:
        page_text = text
    ...
    def parse_online(self, text):
        '''The_Geographical_Journal `online`'''
        # try:
        page_text = text
        tree = etree.HTML(page_text)
        article_title = ''.join(tree.xpath('.//div[@class="article-citation"]//h1[@class="citation__title"]//text()')).strip()
...
def flush(progress_bar):
    ...
    with ThreadPoolExecutor(max_workers=8) as ThreadPool:
        ...
        max_page = 1
        online_urls = [host + f"/action/doSearch?SeriesKey=14754959&sortBy=Earliest&pageSize=20&startPage={page}" for page in range(max_page)]
        ...
        try:
            database_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data\\The_Geographical_Journal.db')
            ...
            for id, article in issue_newest_article_lst:
                if not my_data_manager.search_data(newest_issue, article.title):
                    ...
                    print(f'{article.title:<200} |已存在于 The Geographical Journal_issue')
            print('The Geographical Journal数据存储成功！')
            ...
            for id, article in online_newest_article_lst:
                if not my_data_manager.search_data('online', article.title):
                    ...
                    print(f'{article.title:<200} |已存在于 The Geographical Journal_online')
            print('The Geographical Journal_online数据存储成功！')
        except Exception as e:
            print(e)
