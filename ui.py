...
from mini_ui import thread_with_trace

class MyThread(thread_with_trace):
    def __init__(self, group=None, target=None, who_callback=None, args=None):
        thread_with_trace.__init__(self, group=None, target=target, args=args) # super()方法单继承时只能传递父类所需的参数，此处不用
        if who_callback:
            self.who_callback = who_callback
    
    def run(self):
        super().run()
        if self.who_callback:
            self.who_callback()
...
class Ui_MainWindow(object):
    ...
    def mytranslateUi(self, MainWindow):
        self.datapath = ["data/Annals_of_the_Association_of_American_Geographers.db",
                         "data/The_Professional_Geographer.db",
                         "data/Geoforum.db",
                         "data/Transaction_of_the_Institute_of_British_Geographers.db",
                         "data/The_Geographical_Journal.db"]
    ...
    def click_flush(self):
        ...
        ui = Ui_Form()
        thread_i = MyThread(group=None, target=Flush.flush, who_callback=Form.close,args=(self.comboBox.currentIndex(), ui.refresh_progressbar))
        ui.setupUi(Form, thread_i, journal=self.comboBox.currentText())
        Form.show()
    ...
    def export(self, MainWindow):
        ...
        filepath, _ = QtWidgets.QFileDialog.getSaveFileName(MainWindow, "export_data", os.path.join(os.path.dirname(os.path.abspath(__file__)), filename), 'txt(*.txt)')
        print(filepath)
        if filepath:
            threading.Thread(target=mydata_mgr.export, args=(self.comboBox_2.currentText(), filepath))
